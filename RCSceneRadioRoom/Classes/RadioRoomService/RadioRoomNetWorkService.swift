...
class RadioRoomService {
    ...
    func createRoom(name: String,
                    ...
                    password: String?,
                    roomType: Int,
                    completion: @escaping RCSCompletion) {
    ...
    func userUpdateCurrentRoom(roomId: String, completion: @escaping RCSCompletion) {
        let api = RCRoomService.userUpdateCurrentRoom(roomId: roomId)
        roomProvider.request(api, completion: completion)
    }
    
    func roomInfo(roomId: String, completion: @escaping RCSCompletion) {
    ...
    func setRoomType(roomId: String, isPrivate: Bool, password: String?, completion: @escaping RCSCompletion) {
        let api = RCRoomService.setRoomType(roomId: roomId, isPrivate: isPrivate, password: password)
        roomProvider.request(api, completion: completion)
    }

    func setRoomName(roomId: String, name: String, completion: @escaping RCSCompletion) {
        let api = RCRoomService.setRoomName(roomId: roomId, name: name)
        roomProvider.request(api, completion: completion)
    }
    
    func closeRoom(roomId: String, completion: @escaping RCSCompletion) {
    ...
    func updateRoomBackground(roomId: String, backgroundUrl: String, completion: @escaping RCSCompletion) {
        let api = RCRoomService.updateRoomBackgroundUrl(roomId: roomId, backgroundUrl: backgroundUrl)
        roomProvider.request(api, completion: completion)
    }
    
    func resumeRoom(roomId: String, completion: @escaping RCSCompletion) {
        let api = RCRoomService.resumeRoom(roomId: roomId)
        roomProvider.request(api, completion: completion)
    }

    func suspendRoom(roomId: String, completion: @escaping RCSCompletion) {
        let api = RCRoomService.suspendRoom(roomId: roomId)
        roomProvider.request(api, completion: completion)
    }
    
    func roomList(type: Int = 1, page: Int, size: Int, completion: @escaping RCSCompletion) {
        let api = RCRoomService.roomList(type: type, page: page, size: size)
        roomProvider.request(api, completion: completion)
    }
    
    func checkCreatedRoom(completion: @escaping RCSCompletion) {
        let api = RCRoomService.checkCreatedRoom(type: 2)
        roomProvider.request(api, completion: completion)
    }
    
    func usersInfo(id: [String], completion: @escaping RCSCompletion) {
    ...
    func onlineCreator(completion: @escaping RCSCompletion) {
        let api = RCUserService.onlineCreator(type: 2)
        userProvider.request(api, completion: completion)
    }
